...
assert.match(read('version.php'), /\$plugin->version\s*=\s*2026090605;/, 'Theme version must be bumped for the new visual revision');
...
assert.ok(read('scss/mtpc.scss').includes('background-image: none !important'), 'Default purple Moodle course artwork must be neutralized');
assert.ok(read('scss/mtpc.scss').includes('.dashboard-card::before'), 'Course cards need a stable branded media surface');
